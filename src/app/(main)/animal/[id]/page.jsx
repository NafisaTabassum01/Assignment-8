...
export default async function Page({ params }) {
 ...
 const res = await fetch(
  ...
  return (
    <div className="bg-[#fefae0d6] py-10">
      <div className="w-10/12 mx-auto flex flex-col gap-16 items-center shadow-lg rounded-2xl md:flex-row justify-center bg-[#fffef94b]">
        <Image src={imageMap[animal.image]} alt='' className='md:h-50 w-90'/>
      <div>
     <div>
         <h1 className="text-2xl font-bold text-[#AE6422] pt-5">{animal.name}</h1>
      <p className='badge badge-soft text-[14px] bg-transparent border border-[#AA6122] text-[#AA6122] rounded-2xl mt-2'>{animal.category}</p>
     </div>
      <p className='text-[24px] pt-1.5 font-bold text-green-900'><i className="fa-solid fa-bangladeshi-taka-sign"></i>{animal.price}</p>
      ...
      <div className="w-10/12 mx-auto flex flex-col gap-16 items-center shadow-lg rounded-2xl md:flex-row justify-center bg-[#fffef94b] pt-12 mt-12">
